...
class VisualCardPlugin extends obsidian_1.Plugin {
    cardActionEls = new WeakMap();
    canvasMenuObserver = null;
    timestampTimers = new Map();
    timestampWrites = new Map();
    settings = { ...DEFAULT_SETTINGS };
    async onload() {
        await this.loadSettings();
        (0, obsidian_1.addIcon)(PENCIL_SPARKLES_ICON, PENCIL_SPARKLES_SVG);
        (0, obsidian_1.addIcon)(CANVAS_ADD_ICON, CANVAS_ADD_SVG);
    ...
    refreshCardHeaderActions() {
        const view = this.app.workspace.activeLeaf?.view;
        if (!(view instanceof obsidian_1.ItemView))
            return;
        ...
        handwriting.addClass("visual-card-handwriting-action");
    ...
    async runForActiveVisualCard(action) {
        const files = this.getActiveVisualCard();
        if (!files) {
            new obsidian_1.Notice("Visual Card: Visual CardのMarkdownまたは手書き面を開いてから実行してください。");
            return;
        }
        await action(files);
    }
    resolveVisualCardFromPath(filePath) {
        ...
        if (normalized.endsWith(".excalidraw.md")) {
            ...
            if (card) {
                ...
                return { card, drawing: drawing instanceof obsidian_1.TFile ? drawing : null, drawingPath: normalized, isDrawing: true };
            }
            return null;
        }
        ...
        if (file instanceof obsidian_1.TFile && file.extension === "md") {
            ...
            if (frontmatter?.type === "visual-card" && typeof frontmatter.drawing === "string") {
                ...
                return { card: file, drawing: drawing instanceof obsidian_1.TFile ? drawing : null, drawingPath: frontmatter.drawing, isDrawing: false };
            }
        }
        return null;
    }
    getActiveCanvasView() {
        const view = this.app.workspace.activeLeaf?.view;
        if (view instanceof obsidian_1.ItemView && view.getViewType() === "canvas") {
            return view;
        }
        return null;
    }
    getSelectedCanvasVisualCards() {
        ...
        for (const node of selection) {
            const filePath = node.file?.path ?? (typeof node.filePath === "string" ? node.filePath : node.unknownData?.file);
            if (!filePath)
                continue;
    ...
    updateCanvasNodeMenu(view) {
        const menuEl = view.contentEl.querySelector(".canvas-menu");
        if (!menuEl)
            return;
        ...
        (0, obsidian_1.setIcon)(flipBtn, FLIP_ICON);
        (0, obsidian_1.setTooltip)(flipBtn, "Visual Card: 表裏フリップ");
        ...
        (0, obsidian_1.setIcon)(hwBtn, PENCIL_SPARKLES_ICON);
        (0, obsidian_1.setTooltip)(hwBtn, "Visual Card: 手書き面を編集（Excalidraw）");
        ...
        (0, obsidian_1.setIcon)(textBtn, "file-type-corner");
        (0, obsidian_1.setTooltip)(textBtn, "Visual Card: テキスト面を編集（Markdown）");
        ...
        (0, obsidian_1.setIcon)(copyBtn, "copy-plus");
        (0, obsidian_1.setTooltip)(copyBtn, "Visual Card: カードを複製");
    ...
    recenterCanvasMenu(view, menuEl, target) {
        const canvas = view.canvas;
        if (!canvas || !target?.node)
            return;
        window.requestAnimationFrame(() => {
            try {
                const nodeEl = target.node.nodeEl;
                if (!nodeEl || !menuEl.isConnected)
                    return;
                const nodeRect = nodeEl.getBoundingClientRect();
                const menuRect = menuEl.getBoundingClientRect();
                if (nodeRect.width === 0 || menuRect.width === 0)
                    return;
    ...
    async flipSelectedCanvasCards(silent = false) {
        const view = this.getActiveCanvasView();
        if (!view) {
            if (!silent)
                new obsidian_1.Notice("Visual Card: Canvasを開いてから実行してください。");
            return;
        }
        const targets = this.getSelectedCanvasVisualCards();
        if (targets.length === 0) {
            if (!silent)
                new obsidian_1.Notice("Visual Card: Canvas上で対象のVisual Cardを選択してください。");
            return;
        }
        const canvasFile = view.file;
        if (!(canvasFile instanceof obsidian_1.TFile)) {
            if (!silent)
                new obsidian_1.Notice("Visual Card: Canvasファイルを特定できませんでした。");
            return;
        }
        try {
            ...
            new obsidian_1.Notice(`Visual Card: カードをフリップできませんでした: ${errorMessage(error)}`);
        }
    }
    async duplicateSelectedCanvasCards() {
        const view = this.getActiveCanvasView();
        if (!view) {
            new obsidian_1.Notice("Visual Card: Canvasを開いてから実行してください。");
            return;
        ...
        try {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                const updatedMd = originalMd.replace(/^drawing:\s*.*$/m, `drawing: ${names.drawingCandidatePath}`);
            ...
            new obsidian_1.Notice(`Visual Card: カードの複製に失敗しました: ${errorMessage(error)}`);
        }
    }
    async openCanvasCardHandwriting() {
        const targets = this.getSelectedCanvasVisualCards();
        if (targets.length === 0) {
            new obsidian_1.Notice("Visual Card: Canvas上で対象のVisual Cardを選択してください。");
            return;
        ...
        if (!target.drawing) {
            new obsidian_1.Notice("Visual Card: 手書き面が見つかりません。");
            return;
        }
        await this.app.workspace.getLeaf(false).openFile(target.drawing);
        this.refreshCardHeaderActions();
    }
    async openCanvasCardText() {
        const targets = this.getSelectedCanvasVisualCards();
        if (targets.length === 0) {
            new obsidian_1.Notice("Visual Card: Canvas上で対象のVisual Cardを選択してください。");
            return;
    ...
    getActiveVisualCard() {
        ...
        if (!(activeFile instanceof obsidian_1.TFile))
            return null;
        ...
        if (activeFrontmatter?.type === "visual-card" && typeof activeFrontmatter.drawing === "string") {
            ...
            return { card: activeFile, drawing: drawing instanceof obsidian_1.TFile ? drawing : null, drawingPath: activeFrontmatter.drawing };
        }
    ...
    async openHandwriting(files) {
        if (!files.drawing) {
            new obsidian_1.Notice("Visual Card: 手書き面が見つかりません。『欠損した手書き面を再作成』を実行してください。");
            return;
        }
        await this.app.workspace.getLeaf(false).openFile(files.drawing);
        this.refreshCardHeaderActions();
    }
    async openText(files) {
        await this.app.workspace.getLeaf(false).openFile(files.card);
        this.refreshCardHeaderActions();
    }
    async recreateMissingDrawing(files) {
        if (files.drawing) {
            new obsidian_1.Notice("Visual Card: 手書き面はすでに存在します。");
            return;
        }
        const { folder, stem } = drawingLocation(files.drawingPath);
        if (!stem) {
            new obsidian_1.Notice("Visual Card: drawing のパス形式が不正なため、手書き面を再作成できません。");
            return;
        }
        try {
            if (folder)
            ...
            if (!(drawing instanceof obsidian_1.TFile))
                throw new Error("再作成した手書き面を確認できませんでした");
            new obsidian_1.Notice("Visual Card: 手書き面を再作成しました。");
            await this.app.workspace.getLeaf(false).openFile(drawing);
            this.refreshCardHeaderActions();
        }
        catch (error) {
            console.error("Visual Card drawing recreation failed", error);
            new obsidian_1.Notice(`Visual Card: 手書き面を再作成できませんでした: ${errorMessage(error)}`);
        }
    }
    async addCardToCanvas(files) {
    ...
    async addHandwritingToCanvas(files) {
        if (!files.drawing) {
            new obsidian_1.Notice("Visual Card: 手書き面が見つかりません。先に再作成してください。");
            return;
        }
        await this.addFileToCanvas(files.drawing, "手書き面");
    }
    async addFileToCanvas(fileToAdd, label) {
        const canvases = this.app.vault.getFiles()
            .filter((file) => file.extension === "canvas")
            .sort((a, b) => a.path.localeCompare(b.path));
        if (canvases.length === 0) {
            new obsidian_1.Notice("Visual Card: 追加先のCanvasファイルが見つかりません。");
            return;
        }
        ...
        try {
            ...
            new obsidian_1.Notice(alreadyAdded
            ...
            new obsidian_1.Notice(`Visual Card: Canvasへ追加できませんでした: ${errorMessage(error)}`);
        }
    }
    async openContainingCanvas(files) {
        const canvases = await this.findCanvasesContaining(files);
        if (canvases.length === 0) {
            new obsidian_1.Notice("Visual Card: このカードが追加されているCanvasはありません。");
            return;
        }
        const canvas = await promptForCanvas(this.app, canvases);
        if (!canvas)
    ...
    async findCanvasesContaining(files) {
        const targetPaths = new Set([files.card.path, files.drawing?.path].filter((path) => Boolean(path)));
        const canvases = this.app.vault.getFiles().filter((file) => file.extension === "canvas");
        const matching = [];
        for (const canvas of canvases) {
            try {
                const data = JSON.parse(await this.app.vault.read(canvas));
    ...
    async renameCardTitle(files) {
        const name = cardNameParts(files.card);
        if (!name) {
            new obsidian_1.Notice("Visual Card: ファイル名が YYMMDD-HHmm-タイトル.md 形式ではないため、カード名を変更できません。");
            return;
        }
        ...
        const newPath = (0, obsidian_1.normalizePath)(`${name.folder}${name.folder ? "/" : ""}${name.prefix}-${title}.md`);
        if (newPath === files.card.path)
            return;
        if (this.app.vault.getAbstractFileByPath(newPath)) {
            new obsidian_1.Notice("Visual Card: 同じファイル名がすでにあります。別のカード名を入力してください。");
            return;
        }
        ...
        const newDrawingPath = oldDrawingPath
            ? (0, obsidian_1.normalizePath)(`${drawingFolder}${drawingFolder ? "/" : ""}${name.prefix}-${title}.excalidraw.md`)
            : null;
        if (newDrawingPath && newDrawingPath !== oldDrawingPath && this.app.vault.getAbstractFileByPath(newDrawingPath)) {
            new obsidian_1.Notice("Visual Card: 同じ名前の手書き面がすでにあります。別のカード名を入力してください。");
            return;
        }
        ...
        try {
            ...
            new obsidian_1.Notice(`Visual Card: カード名を「${title}」へ変更しました。`);
            ...
            try {
                ...
                if (drawingRenamed && files.drawing && oldDrawingPath)
                    await this.app.fileManager.renameFile(files.drawing, oldDrawingPath);
                if (cardRenamed)
                    await this.app.fileManager.renameFile(files.card, oldCardPath);
            }
            catch (rollbackError) {
                console.error("Visual Card card rename rollback failed", rollbackError);
            }
            new obsidian_1.Notice(`Visual Card: カード名を変更できませんでした: ${errorMessage(error)}`);
        }
    }
    async updateCanvasFileReferences(oldPath, newPath) {
        const canvases = this.app.vault.getFiles().filter((file) => file.extension === "canvas");
        for (const canvas of canvases) {
            try {
                await this.app.vault.process(canvas, (content) => {
                    const data = JSON.parse(content);
                    if (!Array.isArray(data.nodes) || !data.nodes.some((node) => node.type === "file" && node.file === oldPath))
                        return content;
    ...
    async createVisualCard(rawTitle) {
        const now = new Date();
        const title = normalizeTitle(rawTitle);
        ...
        try {
            await this.ensureFolder(this.settings.cardFolder);
            await this.ensureFolder(this.settings.drawingsFolder);
            markdownFile = await this.app.vault.create(names.markdownPath, createCardMarkdown(title, now, names.drawingCandidatePath));
            ...
            new obsidian_1.Notice(`Visual Card を作成しました: ${names.stem}`);
            window.setTimeout(() => this.refreshCardHeaderActions(), 0);
        }
        catch (error) {
            if (markdownFile) {
            ...
            if (drawingPath) {
                ...
                if (drawingFile instanceof obsidian_1.TFile) {
                    try {
            ...
            new obsidian_1.Notice(`Visual Card を作成できませんでした: ${errorMessage(error)}`);
        }
    }
    async promptAndCreateVisualCard() {
        const title = await promptForTitle(this.app);
        if (title === null)
            return;
        await this.createVisualCard(title);
    }
    async getAvailableNames(now, title) {
        const baseStem = `${formatShortTimestamp(now)}-${title}`;
        for (let suffix = 1;; suffix += 1) {
            const stem = suffix === 1 ? baseStem : `${baseStem}-${suffix}`;
            const markdownPath = (0, obsidian_1.normalizePath)(`${this.settings.cardFolder}/${stem}.md`);
            const drawingCandidatePath = (0, obsidian_1.normalizePath)(`${this.settings.drawingsFolder}/${stem}.excalidraw.md`);
    ...
    async ensureFolder(path) {
        ...
        if (!("children" in existing))
            throw new Error(`${path} はフォルダではありません`);
    }
    async createDrawing(stem, folder, date) {
        const ea = getExcalidrawAutomate();
        if (!ea)
            throw new Error("Excalidraw プラグインが見つかりません。導入・有効化してから再試行してください。");
    ...
    handleVaultModify(file) {
        if (!(file instanceof obsidian_1.TFile) || this.isTimestampWrite(file.path))
            return;
    ...
    getCardForModifiedFile(file) {
        const frontmatter = this.app.metadataCache.getFileCache(file)?.frontmatter;
        if (frontmatter?.type === "visual-card")
            return file;
    ...
    isTimestampWrite(path) {
        const writtenAt = this.timestampWrites.get(path);
        if (!writtenAt)
            return false;
        if (Date.now() - writtenAt < 2000)
            return true;
    ...
    async updateCardTimestamp(card) {
        ...
        try {
            await this.app.fileManager.processFrontMatter(card, (frontmatter) => {
                if (frontmatter.type === "visual-card")
                    frontmatter.updated = formatLocalIso(new Date());
            });
        }
        catch (error) {
...
exports.default = VisualCardPlugin;
class VisualCardSettingTab extends obsidian_1.PluginSettingTab {
    plugin;
...
class CardTitleModal extends obsidian_1.Modal {
    value = "";
    settled = false;
    resolver;
...
class RenameCardTitleModal extends obsidian_1.Modal {
    value;
    settled = false;
    resolver;
    constructor(app, initialTitle, resolver) {
        super(app);
...
class CanvasPickerModal extends obsidian_1.Modal {
    canvases;
    settled = false;
    resolver;
    constructor(app, canvases, resolver) {
        super(app);
...
class CanvasCardSideModal extends obsidian_1.Modal {
    hasDrawing;
    settled = false;
    resolver;
    constructor(app, hasDrawing, resolver) {
        super(app);
...
function normalizedFolder(value, fallback) {
    const normalized = (0, obsidian_1.normalizePath)(value.trim().replace(/^\/+|\/+$/g, ""));
...
function nextCanvasNodeId(nodes) {
...
function createCardMarkdown(title, date, drawingPath) {
